    ...
    if (thread_k_blocks == 16) {
      ...
      for (size_t i = 0; i < thread_m_blocks * 16; i += warps) {
        ...
        hadamard_transform_group_quantize<256, 32, false>(
            ...
            reinterpret_cast<half*>(sh_a_stage) // placeholder
          );
      }
      __syncthreads();
    }
...
// Print all values of sh_s_stage
  // if (threadIdx.x == 0 && blockIdx.x == 0) {
  //   printf("s_sh_stage: %d\n", s_sh_stage);
  //   for (int i = 0; i < s_sh_stage * stages; i++) {
  //     half2* sh_s_stage = reinterpret_cast<half2*>(sh_s);
  //     printf("sh_s_stage[%d]: %f %f\n", i, 
  //            __half2float(sh_s_stage[i].x), __half2float(sh_s_stage[i].y));
  //   }
  // }
  // __syncthreads(); // Ensure all threads complete printing before continuing
...
int marlin_cuda(
  ...
  printf("threshold: %d\n", threshold);
//   printf("thread_m_blocks: %d, total_m: %d, par: %d\n", thread_m_blocks, tot_m, par);
  for (int i = 0; i < tot_m_blocks; i += threshold) {
    int thread_m_blocks = tot_m_blocks - i;
    prob_m = tot_m - 16 * i;
    int par = 1;
    if (thread_m_blocks > threshold) {
